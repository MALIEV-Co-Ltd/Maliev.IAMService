...
The additive `/iam/v1/auth/token-issuance/resolve-permissions` route trusts exactly issuer `https://auth.maliev.com` and audience `https://iam.maliev.com/auth/token-issuance` in every non-test environment. Test-only issuer and audience overrides are isolated to the `Testing` environment. `MaximumLifetimeSeconds` defaults to 30 when omitted, accepts positive values up to 60, and fails closed for malformed, zero, negative, or larger values. IAM enforces zero clock skew and `nbf <= iat < exp` with `exp - iat` bounded by that maximum. Activation also requires at least one RSA public key under `IAM:TokenIssuanceCapability:PublicKeys:{kid}`. IAM accepts RS256 only, requires the configured key ID, and never receives Auth's private signing key.
